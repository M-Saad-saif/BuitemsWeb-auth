...
const UserPortal = () => {
  ...
  const fetchUserData = async () => {
    try {
      ...
      if (response.data.success) {
        ...
        showSnackbar(response.data.error || 'Failed to fetch user data', 'error');
      }
    } catch (error) {
  ...
  const fetchSemesterRecords = async () => {
    try {
      ...
      const response = await axios.get('http://localhost:5000/api/auth/semester-records', {
        headers: { 'auth-token': token }
      });
      ...
      if (response.data.success) {
        ...
        showSnackbar(response.data.error || 'Failed to fetch semester records', 'error');
      }
    } catch (error) {
      console.error('Error fetching semester records:', error);
      showSnackbar(
        error.response?.data?.error || 
        'Failed to fetch semester records', 
        'error'
      );
    } finally {
      setRecordsLoading(false);
  ...
  const handleAddSemester = async () => {
    const invalidSubjects = newSemester.subjects.filter(subject => !subject.name.trim());
    ...
    try {
      const response = await axios.post('http://localhost:5000/api/auth/add-semester', newSemester, {
        headers: { 'auth-token': token }
      });
  ...
  const handleDeleteSemester = async (semesterNumber) => {
    if (window.confirm(`Are you sure you want to delete semester ${semesterNumber}? This action cannot be undone.`)) {
      try {
        const response = await axios.delete(`http://localhost:5000/api/auth/delete-semester/${semesterNumber}`, {
          headers: { 'auth-token': token }
        });
        ...
        if (response.data.success) {
          ...
          showSnackbar(response.data.error || 'Failed to delete semester', 'error');
        }
      } catch (error) {
  ...
  const handleUpdateProfile = async () => {
    ...
    try {
      ...
      if (response.data.success) {
        ...
        showSnackbar(response.data.error || 'Failed to update profile', 'error');
      }
    } catch (error) {
